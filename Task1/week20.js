...
function makeOne() {
	...
	console.log('ggfff');
	try {
		console.log('Попытка вызова functionWithErrors');
...
document.querySelector('.b-1').addEventListener('click', makeOne); //почему не работает?
...
function makeFour() {
	...
	try {
		let number = 4 * 5
		resultFour.textContent = number
	} catch (error) {
		console.error('Ошибка была перехвачена:', error.message);
	}
}

document.querySelector('.b-4').addEventListener('click', makeFour); //не уверена, верно ли поняла
...
function makeFive() {
	let inputData = -1;
	if (inputData < 0)
		throw new Error('Входные данные меньше нуля')
...
function makeNine() {
	const resultEight = document.getElementById('result9');
	try {
		//Тут добавляете try
		let a = 22;
		let c = a + d;
...
function makeTen() {
	...
	try {
		...
		resultTen.textContent = "некорректный email"
	}
...
function makeTwenty() {
	const resultTwent = document.getElementById('result20');
	const resultTwentA = document.getElementById('result20a');
	try {
		resultTwent.textContent = calculateValue()
	} catch (error) {
		console.error(error);
	} finally {
		resultTwentA.textContent = "finally20"
	}
}
...
function makeTwentyFour() {
	...
	try {
		...
		resultTwentyFour.textContent = 'Деление на ноль запрещено'
		console.log(error.name);
		console.log(error.message);
	} finally {
		console.log('Мы знаем, вы пытались');
	}
}
...
function makeTwentyFive() {
	...
	try {
		...
		resultTwentyFive.textContent = `Вы попали в 90%: ${error.message}`;
	}
}
...
function makeTwentySix() {
	...
	} // прим для меня: не выводится catch при асинхронном коде в try. нужно в асинхрон убирать эту конструкцию
...
//Вернуться к заданию.Я вообще не понимаю, что от меня хотят
...
} //Прим для меня: а вот с таким асинхроном работает
...
function makeThirty() {
    ...
    return; //Этот диалог с самой собой для меня Х): а как код будет работать, если после return функция прекращает свою работу? UPD: все, поняла,если убрать return, просто не будет объяснения после нажатия на кнопку, цикл будет бесконечно выполняться
	//Этот тоже: У меня другой вопрос: как добавили этот текст объяснения, после нажатия на кнопку, где скрывается string? UPD: все, нашла, он в другом файле
	try {
		while (true) {
